...
**Trade-offs / honesty:** This is **infrastructure, not reading content** — no new learning rung yet (so the bump is minor, 2.2 → 2.3). The one real design risk is **over-applying "hy"**: "hy" is *exhaustive*, but project-asi is *selective* by its own philosophy (the "rely-on-to-build" test). The merged standard is written into `_CORPUS_BUILD.md` §0/§6 — **ground everything + miss nothing a director needs, but stay at director altitude; don't reproduce the textbook.** Legality is a hard boundary: only legally-free full text or the learner's own owned copies enter the corpus; nothing pirated; the corpus text is never published.
