...
5. **Tooling:** `build_site.py` + generated `agi-paths.html` — a self-contained offline reader over `LEARNING/**.md` in the "hy" style.
...
## v2.3 — 2026-06-27 — *Grounding phase: build a real source corpus (the "sufficiency" fix)*

**What changed:** Started a new production phase. The learner judged the 1000–1300 modules **"not sufficient"** and pointed to their own **"hy" knowledge system** (`C:\Users\cheta\OneDrive\hy`) as the bar. I studied that entire folder and diagnosed the gap precisely: the modules so far are **competent summaries written from general knowledge**, whereas "hy" distils a **corpus of real source text** (`_corpus/`) and **verifies every claim/quote verbatim against it** ("miss nothing → understand deeply → no repetition → simple & beautiful → concise"). The fix is to build the same grounding substrate for project-asi, then re-ground the modules on it. Set up (no downloads yet — gathering deferred to a fresh session per the learner):
1. New [`../RESOURCES/corpus/`](../RESOURCES/corpus/) folder with [`README.md`](../RESOURCES/corpus/README.md) (what/why + git treatment), [`_ACQUISITION_PLAN.md`](../RESOURCES/corpus/_ACQUISITION_PLAN.md) (**full-breadth** GET-list across every AGI/ASI field, two streams — ① extract the learner's already-owned `local_resources/` books, ② fetch legally-free arXiv papers + author-published textbooks + open course notes, with URLs, tiers, and a BUY-list routed to `REQUESTS.md`), [`_CORPUS_BUILD.md`](../RESOURCES/corpus/_CORPUS_BUILD.md) (download/extract/`pdftotext`/chunk method + the **grounded-synthesis standard** + the ▶ next-session START-HERE), and [`_COVERAGE_MAP.md`](../RESOURCES/corpus/_COVERAGE_MAP.md) (status board, all ⬜).
2. `.gitignore` updated so corpus **text** stays local (redistribution/copyright + size, like `local_resources/`) while the corpus **tracking markdown** is published.
3. Project hand-off gained **Task D** (the corpus phase) and the **State** line moved to v2.3.

**Why:** The learner's standing North Star is *understand deeply enough to build/direct AI*. Summaries from memory don't clear that bar — depth that can be **trusted** requires real sources, present as greppable text, with claims traceable to them. Two explicit learner decisions shaped the setup: **(a) gather in a new session** (this session only planned), **(b) full breadth** (every field toward AGI/ASI, not just the core spine).

**Expected benefit:** Future modules — and re-grounded rewrites of 1000–1300 — will stand on verified sources (verbatim quotes, checked numbers, contested/SOTA claims confirmed), which is the "sufficiency" the learner wants. The `PAPERS.md` arXiv-ID verification folds into this phase, so citations become reliable.

**Trade-offs / honesty:** This is **infrastructure, not reading content** — no new learning rung yet (so the bump is minor, 2.2 → 2.3). Legality is a hard boundary: only legally-free full text or the learner's own owned copies enter the corpus; nothing pirated; the corpus text is never published.

**Depth-standard decision (same day, 2026-06-27 — supersedes my initial "director-altitude" framing):** the learner chose the **full, exhaustive hy treatment, even at the cost of length.** Reconciled with the standing philosophy via LEARNER_STRATEGY's own split — *cut implementation depth, never conceptual depth* (§3) — now made explicit in new **§7b**: the **conceptual/understanding axis is EXHAUSTIVE** (render every idea/mechanism/argument/distinction/result/example/trade-off/failure-mode/debate the sources hold, grounded + verbatim, miss nothing), while the **implementation/execution axis stays delegated** (no coding tutorials / proof-grinding). Big topics **split into grounded sub-rung clusters** (numbering gaps), each at ~hy depth (multiple thousands of words). So my earlier "stay at director altitude; don't reproduce the textbook" line is **reversed for the conceptual axis** and updated across `_CORPUS_BUILD.md` §0/§6, `LEARNER_STRATEGY.md` §7/§7b, the hand-off, and memory. The 0100–1300 modules (esp. the flagged 1000–1300) will be **rewritten to this fuller, grounded standard.** One sub-decision left open (not adopted): hy's heavy ESL vocab-glossing layer — add only on request.

---

## v2.4 — 2026-06-27 — *The big vision codified: all-domain corpus + binding HARD_RULES (glossing adopted)*

**What changed:** The learner laid out the full vision and asked that it become **hard, strict rules** — and that **every rule change be consistency-checked** against the existing ones. I read the governing docs first (MASTER_INSTRUCTION, QUALITY_CONTROL_SYSTEM, LEARNER_STRATEGY, DOMAIN_DISCOVERY, RESOURCE_COLLECTION), then codified:
1. **New [`HARD_RULES.md`](HARD_RULES.md)** — the binding collection+synthesis standard (the project's `_DOS_AND_DONTS`, modeled on "hy"). Sections: **§1 SCOPE** (collect a rich corpus across *every crucial domain* affecting humanity, prioritizing AGI/ASI — brain/neuro, AI, quantum, robotics, blockchain/web3, biology, materials, … — studied independently then merged; **all media**: books, papers, courses, blogs, debates, podcasts, YouTube transcripts); **§2 DURABILITY FILTER** (learn the timeless *reason/objective behind* things, **not** transient specifics — we don't memorize the 2012-vs-2020 algorithm, we learn why it exists so we could build the next); **§3 CORPUS** (grounding substrate; legality absolute; text local-only); **§4 concept-by-concept flow + DRY**; **§5 EXHAUSTIVE conceptual depth**; **§6 LANGUAGE** (8-year-old sentences **+ mandatory inline glossing**: general + in-context meaning, simple words, in brackets, for every medium-to-advanced/technical term); **§7 the FINAL GATE** (grounded · complete · non-repeating · glossed · simple · deep · honest — run on every module); **§8 the consistency meta-rule**.
2. **Glossing ADOPTED** (was an open opt-in in v2.3) across `LEARNER_STRATEGY.md` §7b, `_CORPUS_BUILD.md` §6, `QUALITY_CONTROL_SYSTEM.md` §2.
3. **Scope + all-media broadened** in `RESOURCES/corpus/_ACQUISITION_PLAN.md` (+ transcript extraction in `_CORPUS_BUILD.md`).
4. **Registered in the constitution:** `MASTER_INSTRUCTION.md` gains **house rule #10** (grounded/exhaustive/glossed + consistency check) and the HARD_RULES table entry.

**Why:** the learner wants the project to produce, for every crucial field, deep *grounded* understanding of the **enduring** ideas (the kind that survive over time/humanity), in language an 8-year-old can read but with full technical vocabulary kept-and-glossed — and wants this enforced, not aspirational.

**Consistency check (mandated by the new §8 — run this change against the existing rules):** **Consistent / amplifies:** MASTER §4 ("simplify language, never ideas; explain every hard word") → tightened to 8-yr-old + glossing; §5 house rules #5 ("what are we missing") + #8 (DRY) + #3 (nothing final); §8 (contradiction-check before finalize); QC §2 (Consistency check — now made a standing meta-rule); RESOURCE_COLLECTION §3/§5 (Durability/slow-aging) → promoted to the §2 hard filter; LEARNING_ARCHITECTURE (dependency ladder + DRY + numbering-gap insertion) = exactly the §4 concept-by-concept flow. **Two real tensions, resolved on the record:** (a) "collect everything" vs RESOURCE_COLLECTION §1 *anti-hoarding* — resolved by the **corpus exception** (collection is destined for processing, not a trophy shelf; the understanding-per-hour metric now orders *processing*) — amended in that file; (b) "every domain" vs DOMAIN_DISCOVERY *"listed ≠ engaged"* — kept: a domain is "covered" only once it yields grounded modules, not when collected. **One wording fix:** QC/MASTER "12-year-old" → **"8-year-old"** for learning modules (stricter bar, same intent).

**Trade-offs / honesty:** still **infrastructure + rules, not new reading content** (minor bump 2.3 → 2.4). The scope is now very large; the durability filter + leverage-ordered processing are what keep it from sprawling. The heavy glossing + exhaustive depth make modules much longer and slower to write — accepted by the learner. Risk to watch: the all-media net (podcasts/YouTube) must not let unverified claims in — §1.3/§7 require grounding factual claims to primary sources regardless of where the intuition came from.

---

## v2.5 — 2026-07-02 — *The production flow: turning a 31-domain corpus into study material*

**What changed:** After 19 gathering sessions the corpus reached **~54,000 text files across 31 folders**, and the learner asked to **restructure the entire flow of producing study material** — "first understand the vastness and diversity, check the depth and breadth of each, then plan a new robust, optimal, dynamic flow." I surveyed the whole corpus (scale, every folder, sub-folders, a `textbooks/` sample) + re-read the governing docs, then designed and built **v2.5**:
1. **New [`PRODUCTION_FLOW.md`](PRODUCTION_FLOW.md)** — the operating flow, six components: **① Depth Tiers** (Core = exhaustive / Bridge = working depth / Literacy = compact map — calibrated per domain by the rely-on-to-build test × AGI-relevance) · **② The Atlas** (a grounding index re-unifying each domain across the source-organized folders — fixes "no domain lives in one place") · **③ Trunk & Branches** (the single spine becomes the *Trunk* = AGI/ASI narrative; the broad sciences become *Branches* at lighter tiers — because one linear line cannot sequence 31 disciplines) · **④ The Queue** (leverage-ordered "what's next") · **⑤ The assembly line** (the grounded per-module pipeline) · **⑥ Self-healing** (write-pulls-gather; optional external red-team).
2. **New [`../RESOURCES/corpus/_ATLAS.md`](../RESOURCES/corpus/_ATLAS.md)** — the tier-assignment table + a **full depth/breadth audit of all 31 domains** (the learner's "check each" ask) + the source-pool explanation + the slice-growth mechanism. Verdict of the audit: **the corpus is deep+broad in every Core domain and appropriately thin only in Literacy/Bridge — nothing blocks production.**
3. **New [`../LEARNING/_QUEUE.md`](../LEARNING/_QUEUE.md)** — the living queue; top = build `_ATLAS_ai.md` → **1300 LLMs grounded rewrite** (proof-of-standard) → finish the machine-intelligence Core → extend the Trunk (1400–2000) → the Branches.
4. **[`HARD_RULES.md`](HARD_RULES.md) §5.4** (depth tiers) added; §3.1 + header now point to the Atlas + flow. **[`LEARNING_ARCHITECTURE.md`](LEARNING_ARCHITECTURE.md) §14** added (Trunk & Branches refines §11's "one book"). **[`../LEARNING/00_MAP.md`](../LEARNING/00_MAP.md)** relabelled "the Trunk." **[`MASTER_INSTRUCTION.md`](MASTER_INSTRUCTION.md)** doc-index registers PRODUCTION_FLOW.

**Why:** the old flow (one global linear spine + equal exhaustive depth for every module) was built when the corpus was small and AI-only. Against 31 domains it had five structural failures: one line can't sequence 31 disciplines; grounding was scattered across source-organized folders; no depth calibration (⇒ unbounded work); no processing priority; not dynamic. v2.5 fixes each without discarding what worked.

**Consistency check (mandated by HARD_RULES §8):** **Consistent / amplifies:** LEARNING_ARCHITECTURE §11 (folders-vs-one-book) → refined into Trunk & Branches, not contradicted; §8 lifecycle → the assembly line made concrete; §12 chunk pipeline → the Atlas is what "route each chunk" greps against; HARD_RULES §1.4 (order processing by leverage × relevance) → the Queue *is* that rule; §5.1 exhaustive → kept as the **Core** standard. **Tensions resolved on record (in HARD_RULES Reconciliations + PRODUCTION_FLOW §Consistency):** (a) tiers vs. "§5 exhaustive" and vs. "cover every domain" — tiers set *how deep*, not *whether*; exhaustive stays the Core standard; a domain is still "covered" only when it yields grounded modules; (b) just-in-time Atlas vs. "plan before building" — the *system* is scaffolded fully but the *index* is populated per-domain on demand (the leverage rule applied to indexing). **No rule weakened; no silent contradiction.**

**Trade-offs / honesty:** still **infrastructure, not new reading content** (minor bump 2.4 → 2.5) — but this is the last pure-infrastructure step; the next milestone is an actual grounded module (1300). Cost: a little more scaffolding (the Atlas slice) before the first module ships. The tier assignments are judgment calls the learner approved (3-tier scheme, session 20) and can revise. Risk to watch: the Queue and Atlas must stay *used*, not become shelf-ware — the "beware system astronomy" guardrail applies to this machinery too.

**Addendum (2026-07-02, same session) — the AI-proof content test + the consolidated per-file gate.** The learner sharpened the *purpose* of the study material and asked for **one authoritative list of the rules + flow for producing each file, so nothing drifts or contradicts.** Two additions: (1) **[`HARD_RULES.md`](HARD_RULES.md) §2.5 — the AI-proof content test:** the material is *AI-proof knowledge and wisdom that sustains the test of time*, **not** a manual for by-hearting functions/libraries/frameworks/algorithms/definitions/calculations/architecture-templates (all AI-doable). Per-item test: *"understand-and-judge, or reproduce?"* — keep the durable why/trade-off/failure/debate, delegate the recipe. (2) **[`PRODUCTION_FLOW.md`](PRODUCTION_FLOW.md) — "THE PER-FILE GATE":** the 8 rules (Purpose → Depth → Grounding → Structure → Depth-devices → Language → Director-value → Register → Consistency) in one place, each linked to its canonical home (DRY), as the standing anti-inconsistency contract. **Consistency check (§8):** §2.5 sharpens §2.1–§2.3 and shares the conceptual/execution axis-split of §5.2 + LEARNER_STRATEGY §7b — it governs *what kind* of content (durable insight vs. reproducible recipe) while §5 governs *how deep*; a mechanism is still rendered exhaustively **for insight**, just never **as a recipe**. Recorded in HARD_RULES Reconciliations. No rule weakened. Memory: `learning-content-philosophy.md` updated with the sharpened test.

**Addendum 2 (2026-07-02, same session) — extended DRY · dynamic per-file structure + readability · LEARNING folder restructure.** Three more learner refinements: **(1) Extended DRY** (HARD_RULES §4.2 + §7 gate): the no-repetition rule now covers **any idea / argument / theory / example / explanation**, not just "concepts" — each *explained* once in one home, referenced everywhere else, **within a file and across all files**; a repeated *explanation* is a defect (a different *angle/use* and the §4.2a one-line refresher stay fine). **(2) No rigid template + visual reading ease** (HARD_RULES §4.4 rewritten + new §4.5; `_TEMPLATE.md` header; PRODUCTION_FLOW gate Rule 3): the template is now a **flexible palette of required ingredients, not a fixed form** — each file chooses the structure that makes *its* ideas easiest to grasp; break big paragraphs; **length is free** (never trim depth/examples/glosses to shorten). **(3) LEARNING folders restructured into TRUNK + BRANCHES** for the 31-domain corpus: Trunk = `00-foundations` · `10-minds` · `20-machine-intelligence` · `30-math-and-theory` · `40-compute-and-infrastructure` (was `40-compute-and-physical`) · `50-frontier-and-alignment` (was `60-frontier`) · `60-world-and-society` (was `50-…`); NEW Branch libraries `70-sciences/` + `80-engineering/` (per-domain sub-folders created just-in-time). Broken cross-links in the 0250–1300 modules + 00_MAP + README + LANDSCAPE all fixed (data-1600 → 40, governance-2000 → 60); the historical v1.4 entry keeps the old folder names (never overwrite the past). **Consistency check (§8):** extended DRY widens house-rule #8 / §4.2 (same intent) and is reconciled with §4.2a (reminder ≠ re-teach); the flexible template refines §4.4 while keeping every required ingredient mandatory (grounding/glossing/honesty-box/director-value/connections); the folder rename is a **shelf change only** — reading order (sort-keys/00_MAP) and stable `id`s are untouched, so no module content or id-link breaks. No rule weakened.

**Addendum 3 (2026-07-02, same session) — NEW `25-applied-ai-and-agentic-systems/` shelf.** The learner asked whether LEARNING had an "applications" shelf; it didn't (applied content was distributed — agentic in `20`, AI-for-X in the branches). Added a dedicated **Core Trunk shelf `25-applied-ai-and-agentic-systems/`** for the durable applied layer: agentic systems, RAG/grounding, evaluation, AI product/system design, AI-for-X archetypes — **principles, not framework syntax** (§2.5), grounded in the session-17 agentic-engineering corpus (`frontier-rnd/` + papers `D9`). Registered in 00_MAP shelves, LEARNING_ARCHITECTURE §11, README tree, `_ATLAS.md` (Core tier + audit row: "richest applied corpus we hold"), and `_QUEUE.md` (a 4-item applied-AI cluster A–D). `20`'s scope narrowed to the *foundations*; the *applied* layer now lives on `25`. Also clarified extended DRY (commit `95575f7`): the test is **substance, not wording** — a genuinely new viewpoint is welcome, only the same substance in different vocabulary is the forbidden repetition. Consistency: `25` is Core (matches the learner's founder-director goal + the deliberate session-17 pull); it splits cleanly from `20` (foundations) and feeds `BUILDS/` (doing). No rule weakened.

---

## v2.6 — 2026-07-02 — *The Curriculum: the full module blueprint*

**What changed:** The learner asked, before rewriting the thin from-memory modules and producing the rest, for **one place that lays out — robustly, dynamically, in full breadth and depth — the entire set of study-material files to generate: the whole AI domain decomposed into its sub-domains and sub-sub-domains, plus every other field we hold, each at its depth tier.** I surveyed the AI-relevant corpus in depth (papers D1–D16, the good textbooks, the AI courses, the agent-framework docs) to ground the decomposition in what we actually have, then wrote **new [`../LEARNING/CURRICULUM.md`](../LEARNING/CURRICULUM.md)** — the full module blueprint:
- **~185 planned modules** across **PART A — the Trunk** (A0 foundations · A1 minds · A2 AGI-math · **A3 the AI domain decomposed to the bottom** — ML foundations 1000s, deep learning 1100s, RL & agents 1200s, NLP/LLMs 1300s · A4 applied AI & agentic systems · A5 compute & infra · A6 frontier & alignment · A7 world & society) and **PART B — the Branches** (sciences · engineering · robotics/quantum/computer-systems Bridge · blockchain · rest-of-math), each Branch a local ladder + a "how it feeds AGI" capstone.
- Every row carries a **proposed sort-key**, a one-line **durable-core (AI-proof) scope**, a **tier**, and a **grounding pointer** to the corpus folders that back it. The AI domain (A3–A4, ~65 modules) is rendered to the finest granularity because it is the mission's core and the corpus is richest there.
- A **dynamic-growth protocol** (add/split/merge/re-tier by numbering gap) and a **consistency section** are built in.

**Why:** `00_MAP.md` is a ~24-rung reading spine and `_QUEUE.md` holds only the next few items — neither is the *full inventory* the learner needs to see the whole territory and sequence it well. The Curriculum is that inventory: the map the Queue draws from and the Atlas grounds.

**Consistency check (mandated by HARD_RULES §8):** **Division of labor (no duplication):** `00_MAP` = reading order (the book's table of contents) · `CURRICULUM` = the full outline (every leaf module) · `_QUEUE` = leverage-ordered work order · `_ATLAS` = per-domain grounding. The Curriculum expands each `00_MAP` rung into its leaves; it does not restate reading order or grounding. **Tension resolved on record (in CURRICULUM "Consistency" + this entry):** the Curriculum vs. the just-in-time / anti-over-scaffolding rule (PRODUCTION_FLOW ②) — resolved: **a plan of titles is not built files or populated Atlas slices.** Files and Atlas slices remain just-in-time; only the *map* is drawn in full. This is the leverage rule applied to planning — enumeration is cheap and reversible, building is not — so we enumerate freely and build on demand. No rule weakened; no silent contradiction.

**Trade-offs / honesty:** still **planning infrastructure, not new reading content** (minor bump 2.5 → 2.6). The risk is that a big blueprint tempts breadth-first shallow production; the Queue's leverage ordering + the depth tiers are the guardrails (write the highest-value slice deep, not everything thin). Sort-keys inside a cluster are *proposed* — finalized per file at write time (5-digit fine-insertion always available). The blueprint is living: rows will split, merge, and re-tier as writing teaches us more. Memory: `curriculum-blueprint.md` added.
