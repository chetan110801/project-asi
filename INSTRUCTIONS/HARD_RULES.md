...
4.4 **Canonical module skeleton = [`../LEARNING/_TEMPLATE.md`](../LEARNING/_TEMPLATE.md)** (the "hy" file-36 structure — deck → *You are here / Grounded in / Builds on* → concrete idea with `###` staircase → going-deeper → ⚠️ honesty box → director section → Connections incl. "keep three things" + "where to go next" → proof-of-learning → revision notes), **fused** with this project's truth-seeking extras (confidence tags, honesty box, director "delegate vs own", frontmatter, change-tracking). **Exemplar to model on:** the learner's `hy/ayn_rand/Synthesis/36_How_a_Free_Society_Pays_for_Government.md` — adopt its deck, glossing, depth devices (plain-read → principle → what-it-rules-out → why-it-matters; "why this example"; anticipate-the-objection; name-the-mistake), and verbatim `> quote` + `*(Source)*` grounding. Difference: our modules are **multi-source** (so "Grounded in" lists several) and **add** confidence tags + the durability filter.
