...
2. **✅ Grounded.** Every load-bearing claim/number/quote is accurate and **traceable to a real corpus source**; quotes **verbatim** (grep them). Verify; don't hallucinate. Tag confidence. *(v2.4: grounding against [`../RESOURCES/corpus/`](../RESOURCES/corpus/) — [`HARD_RULES.md`](HARD_RULES.md) §3/§7.)*
3. **🗣️ Simple + glossed (ESL).** *(tightened v2.4)* The **8-year-old reading bar** — one idea per sentence, plainest connective words — while ideas stay expert-deep. **Every medium-to-advanced word / phrase / collocation / idiom / technical term is glossed inline** (general + in-context meaning, very simple words, in brackets). Zero bare hard items. ([`HARD_RULES.md`](HARD_RULES.md) §6.)
...
7. **🎯 Director-altitude + EXHAUSTIVE depth.** *(updated v2.4)* Pitched for a founder/director (intuition, trade-offs, when/why). **Conceptual depth is rendered in FULL, not collapsed** — every enduring idea/mechanism/argument/distinction/trade-off/debate the sources hold (miss nothing); **only implementation/execution grind stays delegated** ("direct the AI"). Big topics split into grounded sub-rungs. ([`LEARNER_STRATEGY.md`](LEARNER_STRATEGY.md) §7b, [`HARD_RULES.md`](HARD_RULES.md) §5.)
