...
>
> **✅ Verification log (2026-06-27, corpus gathering session 1):** the **8 spine arXiv IDs** were checked against the live arXiv API (title + first author + year) and are **all correct** — `1512.03385` ResNet, `1706.03762` Attention, `2005.14165` GPT-3, `2001.08361` Kaplan-Scaling, `2203.15556` Chinchilla, `2203.02155` InstructGPT, `1312.5602` DQN, `1707.06347` PPO. These 10 spine papers (+ AlexNet NeurIPS, AlphaGo Nature, both fetched from free copies) are now in [`corpus/papers/`](corpus/).
>
> **✅ Verification log (2026-06-28, corpus gathering session 2):** **~115 more arXiv IDs across D1–D12 were each verified vs the live arXiv API** (returned title/year matched the catalog entry) **and fetched + chunked** into `corpus/papers/<Dn>/`. Covered: D1 extras (word2vec, VAE, GAN, seq2seq, Adam, BatchNorm, VGG, GoogLeNet, U-Net), D2 (Bahdanau, ELMo, BERT, T5, ViT), D3 (Emergent Abilities + Mirage, CoT, Foundation Models, GPT-4 report, Sparks, FLAN, DeepSeek-R1, PaLM, ToT, Self-Consistency, Minerva), D4 (Debate, Unsolved Problems, Christiano DRLHP, Stiennon, Constitutional AI, DPO, Weak-to-Strong, Let's-Verify), D5 (TRPO, DDPG, A3C, SAC, AlphaZero, DreamerV3, OpenAI-Five), D6 (MoE, Switch, Mixtral, RoPE, ALiBi, FlashAttention, Megatron, ZeRO, Speculative, vLLM, LoRA, GPTQ, QLoRA, LLaMA), D7 (all 13 diffusion), D8 (CLIP, Flamingo, Whisper, BLIP-2, LLaVA, ImageBind), D9 (DPR, REALM, RAG, RETRO, Self-RAG, WebGPT, Decision-Transformer, Gato, ReAct, Toolformer, Reflexion, Generative-Agents, Voyager), D10 (S4, RWKV, RetNet, Hyena, Mamba, Jamba), D11 (all 13 robotics incl. RT-1/2, SayCan, PaLM-E, Diffusion-Policy, ALOHA, Open-X, Eureka, OpenVLA, π0), D12 (Med-PaLM, Codex, AlphaCode, SWE-bench, SWE-agent, ChemCrow, GraphCast, Petals, DiLoCo). **All passed.** Still **Nature/Science-only (no arXiv, need free-copy fetch later):** AlphaFold3, RoseTTAFold, ESMFold, AlphaMissense, Evo, AlphaTensor, AlphaDev, FunSearch, AlphaGeometry, tokamak-control, GNoME, GenCast, NeuralGCM, AlphaQubit, AlphaChip, RFdiffusion, Coscientist, Genie, TabPFN, the astronomy set. (AlphaFold2 free copy already fetched → `corpus/biology/`.)
