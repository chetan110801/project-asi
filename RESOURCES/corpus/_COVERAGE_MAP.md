...
**Totals (after gathering session 13, 2026-07-01):** **~1,867 discrete book/paper/course sources** (1,841 + 16 papers + 6 books + 4 courses) plus the transcript corpus now at **134 lecture courses + ~4,532 podcast/lecture/channel transcripts**. D13 L4 survey layer now covers **every** major AI sub-domain (the session-12 set + NLP/CV/speech/meta-learning/transfer/uncertainty/OOD/neuro-symbolic/NAS/PEFT/prompting/hallucination/offline-RL/multiagent-RL); the residual textbook ⬜ sub-domains (computational biology, immunology, orbital mechanics, hydraulics, power systems, embedded systems) are filled, with comms + VLSI routed to REQUESTS. Still gathering — **do NOT write modules** until the learner calls the corpus rich enough. **Next ▶ session 14:** (a) recover the 4 transient-429 8.701 clips + a 3rd L4 arXiv batch if any AI/science sub-domain remains thin; (b) more deep-tail `@mitocw`/`@stanfordonline` gap courses (validated IDs in [`_UNIVERSITY_PLAYLISTS.md`](_UNIVERSITY_PLAYLISTS.md)); (c) any remaining ⬜ books (developmental biology, dedicated propulsion, transportation civil); (d) OCR the 3 image-only scans if an OCR stack is added — **or**, if the learner calls the corpus rich enough, the first **grounded module rewrite (1300 LLMs)**. Method ▶ [`_CORPUS_BUILD.md`](_CORPUS_BUILD.md).
