...
## Gathering session 16 (2026-07-01): Stream-3 YouTube talk channels + NASA-NTRS-per-field + bonus org feeds + media channels (Cleo Abram, MLST)
Did the two **session-15 DEFERRED** items first (NASA NTRS PDFs + YouTube talk channels one-at-a-time), then bonus feeds + the learner's new media requests. All 0 FFFD; **YouTube pulled ONE channel at a time** (no 429 storms; a few long-talk 100 s socket-timeouts self-recovered on the 60 s retry). Toolchain recovered from the session-15 scratchpad (`db3c70b9`): `ytchannel.sh`+`vtt2txt.pl`, `getpdf.sh`, `rndfeed.sh`+`rss2txt.pl`, `norm.pl`, `chunk.sh`.
- **(a) Frontier-org YouTube talk channels → NEW `frontier-rnd/<org>-youtube/`:** **Neuralink** `neuralink-youtube` **14** (⭐ learner-named — Summer-2025 / July-2024 update talks; filter ≥140 s) · **Boston Dynamics** `boston-dynamics-youtube` **36** (3 humanoid tech-talks/webinars 1351/1075/895 lines + Inside-the-Lab Atlas + RL behaviour; filter ≥240 s, music demos skipped) · **Microsoft Research** `microsoft-research-youtube` **43** (recent 20–90 min research talks: diffusion theory, quantum sim, materials inverse-design, FHE, AI-reasoning, BCIs, health-AI). *(en-en.vtt empty-file race hit 4 BD clips → deleted + re-pulled clean; carry-forward gotcha below.)*
- **(b) NASA NTRS — 13 NEW public-domain reports / 35 chunks → `frontier-rnd/nasa-ntrs-*` (now 19):** ML-aircraft-engine-design · composite-virtual-testing · electric-propulsion-AEPS `6` · CFD-progress-survey · **astrobiology+AI (Scharf 2026)** · ECLSS-overview · **quantum-sensing (2025)** `5` · autonomous-operations · nuclear-propulsion-101 · optical-comms (GEO-relay) · Mars-rover-autonomy · ISRU · resilient-airspace-autonomy. Method: search-API `ntrs.nasa.gov/api/citations/search?q=…` → **resolve each result's real `downloads/<file>.pdf` link** (many have descriptive filenames, not `<id>.pdf`) → `getpdf.sh`. Swapped one image-only scan (deep-space-optical) for the digital-native GEO-relay copy.
- **(c) BONUS org feeds — 8 NEW / 26 chunks** (probed for real RSS/Atom vs SPA shells): **Argonne** `2` · **ORNL** `6` · **SLAC** `7` · **Jefferson Lab** `4` · **Intel newsroom** `1` · **HHMI Janelia** `1` · **Cold Spring Harbor** `2` · **Together AI** `3`. **`frontier-rnd/` now 80 folders** (was 56).
- **(d) Media channels (learner-requested) → `transcripts/`:** **Cleo Abram** `cleo-abram` **60** full-length videos (all 60 got, 0 nosub — DeepMind-CEO/Altman-GPT5/Jensen-Huang/Zuckerberg/Doudna/Brian-Cox interviews; Shorts tab not pulled) · **Machine Learning Street Talk** `machine-learning-street-talk` **recent 50 eps (down-payment)** (John Jumper/AlphaFold, Michael Jordan, Jeremy Howard, Yi Ma, Pedro Domingos, Llion Jones; ≥20 min).
- **CARRY-FORWARD GOTCHAS:** (1) **en-en.vtt empty-file race** — when yt-dlp writes both `<id>.en.vtt` and `<id>.en-en.vtt`, an occasional pass yields an empty transcript; if a killed foreground run truncated a file, the resume skips it. Fix used: after a pull, delete any `<5`-line transcript + its `.done_<id>` marker and re-run (resumable). (2) NTRS filenames often aren't `<id>.pdf` → hit `/api/citations/<id>` and read `downloads[].links.pdf`. (3) some NTRS PDFs are image-only scans (tiny text) → check text_bytes, swap for a digital copy. (4) run YouTube channels strictly one-at-a-time; enumeration is also a YouTube hit — don't enumerate while a pull runs.
- **⏭ DEFERRED to next session (learner added these mid-session):** **(1) NVIDIA Developer + IBM Research** YouTube channels. **(2) Scrape top AI-lab sites + docs** (OpenAI/Anthropic/DeepMind/Meta/Mistral/xAI) — TESTED: Anthropic **`docs.anthropic.com/llms-full.txt` = 86.8 MB whole-docs markdown** (+ `docs.claude.com` mirror; `llms.txt` 202 KB index) · xAI `docs.x.ai/llms.txt` 1.23 MB · OpenAI `developers.openai.com/llms.txt` 90 KB · Gemini `ai.google.dev/gemini-api/docs/llms.txt` 27 KB · Mistral `docs.mistral.ai/llms.txt` 14 KB. Live sitemaps: OpenAI(3 KB idx)/Anthropic(63 KB)/DeepMind(81 KB)/xAI(16 KB). **(3) Y Combinator** — YouTube (Startup School / How-to-Start-a-Startup / founder talks) + YC Library essays + Paul Graham essays. **(4) Rich AI/eng/science podcasts** — full **Lex Fridman** channel curated to AI/sci/eng guests + finish MLST + No-Priors/Latent-Space/Cognitive-Revolution/80k-Hours/TWIML/Mindscape/Huberman/Robot-Brains/AXRP/a16z.

**Totals (after gathering session 16, 2026-07-01):** **~1,951 discrete book/paper/course/R&D sources** (1,930 + 13 NTRS + 8 feeds) across **80 `frontier-rnd/` folders**, plus the transcript corpus at **134 lecture courses + ~4,739 transcripts** (+93 frontier-org YouTube: Neuralink 14 / Boston-Dynamics 36 / MSR 43; +60 Cleo Abram; +50 MLST). Still gathering — **do NOT write modules** until the learner calls the corpus rich enough. **Next ▶ session 17:** the 4 DEFERRED items above (NVIDIA/IBM YouTube · AI-lab site+docs scrape via llms-full.txt/sitemaps · Y Combinator · full Lex + podcast wave), then low-priority Stream-2 residuals → **pause for the learner's "rich enough" call → first grounded module rewrite (1300 LLMs)**.

## Gathering session 17 (2026-07-01): ⭐ AI-lab official docs + END-TO-END AGENTIC-ENGINEERING corpus (learner-directed mid-session)
The learner redirected the session to **"current official state-of-the-art docs + all info from LangChain/LangGraph/LangSmith/AutoGen/etc. + all available SOTA in-depth on end-to-end agentic engineering — the dynamic Agentic-Engineering system-design discipline replacing 'vibe-coding' trending online."** Executed as a large clean docs wave (mostly single `llms-full.txt` / `llms.txt` fetches + official GitHub docs repos + SSR blog de-chroming). **38 NEW `frontier-rnd/` folders · 4,386 verbatim chunks · ALL 0 FFFD · 0 oversized chunks.** `frontier-rnd/` now **118 folders / 163 MB** (was 80). All text git-ignored.
- **(A) AI-lab official docs (5) — the builder-facing documentation:** **Anthropic/Claude** `anthropic-docs` **1734** (⭐ `docs.anthropic.com/llms-full.txt` = 86.8 MB whole-docs-as-markdown; CHUNK_BYTES=50000) · **OpenAI** `openai-docs` **268** (`developers.openai.com/llms-full.txt` 3.9 MB) · **xAI** `xai-docs` **87** (`docs.x.ai/llms.txt` 1.23 MB full) · **Mistral** `mistral-docs` **59** (`llms-full.txt`) · **Gemini** `gemini-docs` **1** (`ai.google.dev/gemini-api/docs/llms.txt` index; no llms-full). [Cohere = 534-byte stub, skipped.]
- **(B) LLM/agent-framework docs (16) via llms.txt/llms-full.txt:** **LangChain (unified LangChain+LangGraph+LangSmith)** `langchain-docs` **415** (`docs.langchain.com/llms-full.txt` 12.66 MB; LangGraph coverage verified — 313 files ref langgraph/StateGraph/checkpointer/create_react_agent) · **LangSmith** `langsmith-docs` **21** (folded minified lines then re-chunked) · **CrewAI** `crewai-docs` **126** · **Haystack** `haystack-docs` **120** · **Pydantic AI** `pydantic-ai-docs` **151** · **Google ADK** `google-adk-docs` **159** · **Letta (MemGPT)** `letta-docs` **55** · **LlamaIndex** `llamaindex-docs` **7** (index) · **LiteLLM** `litellm-docs` **39** · **Agno** `agno-docs` **334** (10.2 MB) · **MCP (Model Context Protocol)** `mcp-docs` **137** (1.96 MB) · **DSPy** `dspy-docs` **1** · **OpenAI Agents SDK** `openai-agents-sdk-docs` **2** · **Mastra** `mastra-docs` **2** · **Instructor** `instructor-docs` **1** · **Weaviate** `weaviate-docs` **3**.
- **(C) Microsoft agent frameworks (3) — no llms.txt → official GitHub docs repos (fetch raw .md + .ipynb):** **Semantic Kernel** `semantic-kernel-docs` **131** (MicrosoftDocs/semantic-kernel-docs, MIT; 135/137 md) · **Microsoft Agent Framework** `microsoft-agent-framework-docs` **81** (microsoft/agent-framework, MIT; incl. `docs/decisions/` ADRs = system-design rationale, gold) · **AutoGen** `autogen-docs` **43** (microsoft/autogen, CC-BY-4.0; 79 user-guide notebooks+md, ipynb→text via `ipynb2txt.py`).
- **(D) Agentic-engineering METHODOLOGY — the anti-vibe-coding / system-design canon (7):** **OpenAI "A Practical Guide to Building Agents"** `openai-agents-guide` **3** (whitepaper PDF; text-sparse visual guide) · **Anthropic Engineering blog** `anthropic-engineering` **11** (21 SSR articles enumerated from `anthropic.com/sitemap.xml` /engineering/ → de-chromed: building-effective-agents, effective-context-engineering, writing-tools-for-agents, effective-harnesses-for-long-running-agents, multi-agent-research-system, demystifying-evals-for-ai-agents, code-execution-with-mcp, agent-skills, claude-code-best-practices, advanced-tool-use — NOT in the product docs) · **12-Factor Agents** `twelve-factor-agents` **6** (humanlayer/12-factor-agents; the 12 production-agent factors + brief-history) · **GitHub Spec Kit / Spec-Driven Development** `spec-kit-sdd` **23** (github/spec-kit, MIT; SDD concepts + command/spec/plan/tasks templates — the disciplined replacement for vibe-coding) · **Latent Space** `latent-space-blog` **24** (swyx/Alessio AI-Engineering feed) · **Simon Willison** `simon-willison-blog` **5** · **Chip Huyen** `chip-huyen-blog` **17**.
- **(E) Agentic infra/observability/eval + more platforms (7):** **AWS Strands Agents** `strands-agents-docs` **124** (2.5 MB) · **Cloudflare Agents** `cloudflare-agents-docs` **131** (1.89 MB) · **HF smolagents** `smolagents-docs` **20** · **n8n** `n8n-docs` **39** (low-code agentic workflows) · **Dify** `dify-docs` **3** · **Langfuse** `langfuse-docs` **1** (LLM/agent observability & eval) · **Ragas** `ragas-docs` **2** (RAG/agent evaluation).
- **TOOLS BUILT (scratchpad, reusable):** `gettext.sh` (plain-text/markdown llms.txt fetcher → norm → chunk, honors CHUNK_BYTES) · `ghfetch.sh` (GitHub raw .md/.ipynb list → **fetch-all-then-concat-in-ONE-redirect** [lock-safe] → norm → chunk) · `ipynb2txt.py` (notebook markdown+code cells → text) · `htmlpages.sh` (SSR page-URL list → de-chrome `html2txt.pl` → single-redirect concat → chunk). Reused: `getpdf.sh`, `rndfeed.sh`+`rss2txt.pl`, `norm.pl`, `chunk.sh`, `html2txt.pl`.
- **CARRY-FORWARD GOTCHAS:** (1) the Windows **append-lock** ("Device or resource busy") recurs on rapid `>>` reopen in fetch loops → the fetchers now do Phase-1 fetch-all-to-cache, Phase-2 concat under a **single** `> CAT` redirect. (2) **minified `llms.txt`** (e.g. LangSmith) = few very long lines → blank-line chunking makes one giant chunk; fold with `perl -pe 's/(.{800,}?) /$1\n/g'` before `chunk.sh`. (3) **AutoGen & Semantic Kernel have no llms.txt** → use their official docs GitHub repos (raw.githubusercontent, no API rate-limit; list files via one `git/trees/<branch>?recursive=1` API call). (4) **Anthropic engineering blog is server-rendered** (≈5.9k words/article) and is **separate from `docs.anthropic.com`** (product docs) → enumerate the sitemap `/engineering/` + de-chrome. (5) **Google "Agents" whitepaper** is Kaggle-gated JS (no clean PDF found) → REQUESTS candidate. (6) GitHub unauth API = 60 req/hr — list files via ONE trees call/repo, then fetch raw (unlimited).

- **(F) RAG / vector-DB + retrieval infra (5, agentic memory/retrieval layer):** **Chroma** `chroma-docs` **57** · **Qdrant** `qdrant-docs` **147** · **Pinecone** `pinecone-docs` **3** · **Milvus** `milvus-docs` **3** · **HF AI-Agents Course** `hf-agents-course` **1** (index). [Google "Agents"/"Agents Companion" whitepapers = Kaggle-gated SPA, no clean PDF → `REQUESTS.md`.]
**Totals (after gathering session 17, 2026-07-01):** **~1,994 discrete book/paper/course/R&D sources** (1,951 + 43 agentic-eng/AI-lab doc sources) across **123 `frontier-rnd/` folders (~164 MB)**, plus the transcript corpus at 134 lecture courses + ~4,739 transcripts. This session deepened the **"build & direct AI" + end-to-end agentic-engineering** axis — the most directly North-Star-relevant material yet (official docs of every frontier AI lab + 26 agent frameworks/platforms + the agentic-engineering-vs-vibe-coding methodology canon). Still gathering — **do NOT write modules** until the learner calls the corpus rich enough. **Next ▶ session 18:** the original session-17 Stream-3 items still open (NVIDIA/IBM-Research YouTube · Y Combinator: YouTube + YC Library + Paul Graham essays · full Lex Fridman curated + finish MLST + podcast wave) + optional agentic-eng additions (Google agents whitepaper if a route opens, DeepLearning.AI/HF Agents course, Berkeley LLM-Agents MOOC, Karpathy "Software 3.0"/vibe-coding talk transcript) + low-priority Stream-2 residuals → **pause for the learner's "rich enough" call → first grounded module rewrite (1300 LLMs)**.

## Gathering session 18 (2026-07-01): the still-open session-16/17 Stream-3 items — NVIDIA + IBM Research YouTube · Y Combinator (Paul Graham essays + YC YouTube) · rich AI/sci/eng podcast wave
Executed the DEFERRED item-1/3/4 set from sessions 16–17. All **0 FFFD**; **YouTube pulled ONE channel at a time** (no 429 storms — NVIDIA & YC clean; IBM hit 3 transient 429s, backed-off/handled) while HTTP text-podcast pulls ran **in parallel on other hosts** (safe — different servers, lock-safe fetch-all-then-single-redirect concat). Toolchain rebuilt from session-17 scratchpad (`32780f4c`): `ytchannel.sh`+`vtt2txt.pl`, `htmlwalk.sh`/`html2txt.pl`, `norm.pl`, `chunk.sh`; **NEW `bulkwalk.sh`** (generic lock-safe HTTP walker: Phase-1 fetch-all-to-cache → Phase-2 concat under ONE redirect → chunk + QC).
- **(A) Frontier-org YouTube talk channels (≥15 min filter) → NEW `frontier-rnd/<org>-youtube/`:** **NVIDIA Developer** `nvidia-developer-youtube` **248** (of 249 enumerated ≥15 min; 1 no-caption) — full GTC technical sessions & deep-dives: humanoid robotics, CUDA/Blackwell/Dynamo architecture interviews, quantum computing, agents, Cosmos world-models, digital twins/CERN, deep-learning course · **IBM Research** `ibm-research-youtube` **187** (of 203 ≥15 min; ~13 no-caption, 3 transient-429 left for re-run) — quantum-computing roadmap/algorithms/utility, AI agents, AI safety/cryptography, quantum-centric supercomputing, semiconductors/EDA, NASA-IBM science-AI.
- **(B) Y Combinator (item 3) → `frontier-rnd/`:** **Paul Graham essays** `paul-graham-essays` **125 chunks** (234 essays, `paulgraham.com` static HTML → `htmlwalk`; the founder/startup-wisdom canon) · **Y Combinator YouTube** `y-combinator-youtube` **469** (of 478 enumerated ≥15 min — Lightcone Podcast, How to Build the Future, Startup School lectures, founder interviews). **YC Library** (`ycombinator.com/library`) is a hard SPA (content via client-side API, no embedded body) and its items are mostly founder-interview **videos** → routed via the YC YouTube channel (per the "switch route, don't de-chrome a SPA" rule).
- **(C) Rich AI/sci/eng PODCAST wave — full-text transcripts from the shows' own sites (higher quality than auto-captions), chunked → NEW `transcripts/<slug>/`:** **AXRP** (AI X-risk Research Podcast, Daniel Filan) `axrp-podcast` **150 chunks** (62 eps, `axrp.net` — pure alignment research) · **80,000 Hours** `80000-hours-podcast` **495 chunks** (116 eps — AI/AGI/x-risk/biosecurity/governance subset of 276, slug-filtered: Ajeya Cotra, Carl Shulman, Holden Karnofsky, Buck Shlegeris, Beth Barnes, Daniel Kokotajlo AI-2027, Helen Toner, Dean Ball…) · **The Cognitive Revolution** (Nathan Labenz) `cognitive-revolution-podcast` **939 chunks** (357 non-newsletter eps — AI/agents/frontier-models/alignment, each post carries a full `TRANSCRIPT` section) · **Sean Carroll's Mindscape** `mindscape-podcast` **333 chunks** (144 eps — science/mind/computation/complexity/physics-of-information subset of 434, slug-filtered: Rovelli, Chalmers, Penrose, Susskind on quantum information, Melanie Mitchell on AI, Friston free-energy, Sara Walker on information & origin of life, Tegmark, Strogatz).
- **METHOD / GOTCHAS (carry-forward):** (1) **NVIDIA/IBM/YC channels are huge (600–1000+ videos)** → enumerate with `%(duration)s` and duration-filter to substantial talks (≥15 min = the GTC sessions / Lightcone eps / lectures) to skip short promos/tutorials. (2) IBM's talks are long (30–60 min) so the pull is slow (~5 videos/min) — budget ~35 min for 200; it's resumable. (3) **HTTP text-podcast pulls parallelize safely with a YouTube pull** (different hosts) — but keep to ≤2 concurrent to avoid the Windows append-lock; `bulkwalk.sh` uses fetch-all-to-cache + single-redirect concat so the lock never bites. (4) **80k Hours & Mindscape are broad** (much non-AI) → slug-keyword filter to the on-mission subset (filter the SLUG only — the domain `preposterousuniverse.com` contains "universe" and false-matches a naive full-URL grep). (5) YC Library & most Substack "archive" pages are SPAs → use the `sitemap-posts.xml` (CR: Ghost CMS, posts at root `/<slug>/` with a full `TRANSCRIPT` block) or route via YouTube. (6) The en-en.vtt empty-file race + one-channel-at-a-time rules from session 16 still hold.

**Totals (after gathering session 18, 2026-07-01):** **~2,002 discrete book/paper/course/R&D sources** (1,994 + 4 new `frontier-rnd/` folders + 4 new `transcripts/` podcast corpora) across **127 `frontier-rnd/` folders**, plus the transcript corpus at **134 lecture courses + ~5,643 YouTube transcripts** (+904 this session: NVIDIA-Developer 248 / IBM-Research 187 / Y-Combinator 469) **+ ~679 full-text podcast-episode transcripts** (AXRP 62 · 80,000 Hours 116 · Cognitive Revolution 357 · Mindscape 144 → 1,917 chunks) **+ 234 Paul-Graham essays → 125 chunks**. Session-18 new-chunk total (chunked sources) = **2,042**. This session closed the frontier-org **YouTube talk-channel** axis (NVIDIA + IBM) + the **founder/"build-a-company"** axis (Paul Graham + YC) + a large **AI/x-risk/science podcast** layer (AXRP, 80k, Cognitive-Revolution, Mindscape). Still gathering — **do NOT write modules** until the learner calls the corpus rich enough. **Next ▶ session 19:** the podcast-wave remainder (full **Lex** curated to AI/sci/eng · finish **MLST** · No-Priors / TWIML / a16z / Robot-Brains / Latent-Space-podcast / Huberman) + IBM/YC rate-limited-clip re-runs → **pause for the learner's "rich enough" call → first grounded module rewrite (1300 LLMs)**.

## Gathering session 19 (2026-07-01): the podcast-wave remainder — full Lex (AI/sci/eng) + finish MLST + No-Priors + Robot Brains + TWIML + Latent Space podcast
Executed the session-18 DEFERRED podcast wave. All **0 FFFD**; **YouTube pulled ONE channel at a time** (the session-18 throttle had cooled — confirmed by a fresh single-video caption probe; No-Priors & Robot-Brains each left 1 transient-429 clip, MLST & TWIML clean at 0-left) while an **HTTP text-podcast pull ran in parallel on another host** (`bulkwalk.sh`, lock-safe). Toolchain rebuilt from the session-18 scratchpad (`1f6ee772`): `getts.sh`, `bulkwalk.sh`, `ytchannel.sh`+`vtt2txt.pl`, `html2txt.pl`, `norm.pl`, `chunk.sh`.
- **(A) ⭐ Lex Fridman — AI/sci/eng CURATED, via lexfridman.com human-generated transcripts (higher quality than auto-captions) → NEW `transcripts/lex-<guest>/` = 35 folders / 409 chunks:** enumerated the podcast page's 115 recent `-transcript` slugs, filtered to AI/science/engineering/frontier-tech guests (excluding the 7 already held + all politics/history/comedy/sports/music/culture). **AI/CS/eng/tech (21):** ai-sota-2026, cursor-team, deepseek (Dylan Patel + Nathan Lambert), Elon-Musk-4, Elon-Musk + Neuralink-team, George-Hotz-3, Guillaume-Verdon, Joscha-Bach-3, Marc-Andreessen-2, Sundar-Pichai, Terence-Tao, ThePrimeagen, Tim-Sweeney, ffmpeg-team, Pieter-Levels, Peter-Steinberger, DHH, Dave-Plummer, Jeff-Bezos, Marc-Raibert, David-Kirtley (fusion). **Science (14):** Adam-Frank, Charan-Ranganath, Don-Lincoln, Edward-Gibson, Janna-Levin, Joel-David-Hamkins, Lee-Cronin-3, Lisa-Randall, Michael-Levin-2, Neri-Oxman, Nick-Lane, Sara-Walker-3, Sean-Carroll-3, Yuval-Noah-Harari. (Older classic AI eps — Karpathy/Ilya/Bengio/Hawkins/Russell — have NO website transcript, only YouTube auto-caps → deferred, low-priority.)
- **(B) Finish MLST — full-channel sweep → `machine-learning-street-talk/` = 242 (was 50; +192 new):** enumerated all 251 videos, filtered ≥15 min (244), skip-existing dropped the 50 held → 192 new, 2 no-caption, **0 rate-limits**.
- **(C) NEW YouTube AI-podcast channels, one-at-a-time → NEW `transcripts/<slug>/`:** **No Priors** (Sarah Guo & Elad Gil) `no-priors-podcast` **167** (of 168 ≥15 min; 1 transient-429) · **The Robot Brains** (Pieter Abbeel) `robot-brains-podcast` **64** (of 65 full eps ≥15 min; 44 clips/shorts skipped; 1 transient-429) · **TWIML AI Podcast** (Sam Charrington) `twiml-ai-podcast` **248** (most-recent 250 of ~1,000 ≥15-min eps — the 2023-2026 LLM/agent-era interviews; 2 no-caption; older back-catalog ~750 eps deprioritized as dated AI-news era per the durability filter).
- **(D) Latent Space: The AI Engineer Podcast — expansion via latent.space Substack SSR posts → NEW `transcripts/latent-space-podcast/` = 408 chunks / 295 posts:** enumerated `sitemap.xml` (413 `/p/` posts), excluded the 107 `ainews-*` daily-newsletter digests (transient news) + event/ticket promos → 295 interview + technical-essay posts (Jeff-Dean, DeepSeek, Cognition, dreamer, boltz, ESMFold2, materials, swe-bench-dead, forward-deployed-engineers…). Complements the session-17 `frontier-rnd/latent-space-blog` (24).
- **(E) IBM Research YouTube — recovered 1 of the 3 session-18 rate-limited clips** (QVECTOR variational-QEC talk, 627 lines) → `frontier-rnd/ibm-research-youtube/` now **188**; 2 left (1 Spanish-language = out-of-scope for the English corpus, 1 persistent-429).
- **SKIPPED / assessed (durability + reachability filter):** **a16z** — podcast pages are show-notes only (no transcripts, ~15 KB de-chromed) + topics heavily off-theme (crypto/bio/fintech) → skipped · **Huberman Lab** — neuroscience, low-priority (corpus already deep in neuro: Kandel / Neuronal-Dynamics / MIT-9.x / Kirsanov) → deferred · **No-Priors / TWIML / Robot-Brains sites** — no HTTP transcripts (No-Priors host unreachable; TWIML pages have no transcript block) → routed via YouTube · **Karpathy "Software Is Changing (Again)"** talk — already held (`y-combinator-youtube` LCEmiRjPEtQ + `dwarkesh-andrej-karpathy`).
- **CARRY-FORWARD GOTCHAS:** (1) the YouTube throttle is **per-video-ID + transient**, not a blanket IP block — a fresh single-video caption probe tells you if it has cooled; a few clips can 429 through even the 60 s backoff (left unmarked, resumable). (2) lexfridman.com human transcripts live at `/<guest-slug>-transcript/` (recent eps only; older 404) — `getts.sh` de-chromes them fine (minor nav boilerplate, 0 FFFD). (3) latent.space Substack **post** pages are SSR (full transcript embedded); only the **archive** page is a SPA → enumerate via `sitemap.xml`, exclude `ainews-*`. (4) TWIML `/videos` back-catalog is 1,000+ eps → take a recent slice (reverse-chron `head -N`).

**Totals (after gathering session 19, 2026-07-01):** **~2,042 discrete book/paper/course/R&D sources** (2,002 + 4 new `transcripts/` podcast corpora [latent-space-podcast · no-priors-podcast · robot-brains-podcast · twiml-ai-podcast] + 35 Lex episode folders; IBM +1) across **127 `frontier-rnd/` folders** (IBM now 188), plus the transcript corpus at **134 lecture courses + ~6,315 YouTube transcripts** (+672 this session: MLST +192 / No-Priors 167 / Robot-Brains 64 / TWIML 248 / IBM +1) **+ a full-text podcast/essay layer** (Lex 35 eps → 409 chunks · latent-space-podcast 295 posts → 408 chunks; on top of session-18's AXRP/80k/CR/Mindscape). This session closed the **AI/sci/eng interview-podcast** axis (Lex · MLST · No-Priors · Robot-Brains · TWIML) + the **AI-engineering practitioner** axis (Latent Space). Still gathering — **do NOT write modules** until the learner calls the corpus rich enough. **Next ▶ session 20:** the corpus is now very broad across all 3 streams — **recommend pausing for the learner's "rich enough" call → the first grounded module rewrite (1300 LLMs)**. Optional residual adds if wanted: Huberman curated · TWIML older back-catalog · Berkeley LLM-Agents MOOC · DeepLearning.AI / HF Agents courses · the 4 remaining transient-429 clips (IBM 2 / No-Priors 1 / Robot-Brains 1) + low-priority Stream-2 residuals.
