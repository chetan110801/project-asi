...
### R5c — COURSE WIDENING → `corpus/courses/` (**29 courses · 998 transcripts**; course corpus now **91 folders · 2,615 transcripts · 96 MB**)
| Group | Courses | Transcripts |
|---|---|---|
...
> **New domains reached this session:** probability/statistics (Stat110, 6.041), quantum (8.05/8.06), physics (8.01/8.03), economics (14.01/14.02), biology (7.016), robotics/control (6.832, 2.003SC, Brunton control/dynamics), neuroscience (9.14, Kirsanov), cognitive-science (24.08J), applied-math/calculus (18.02, 3B1B, Brunton SVD/DiffEq), PGM (10-708), decision/uncertainty. **+4 universities** (Harvard, UMich, Cornell, Tübingen) + 2 expert channels (Brunton, Kirsanov). Benign empties only (Berkeley CS188 NA/private placeholders; 8.06/8.01 flat-list-capped at ~100; scattered caption-less clips). Playlist-ID table: [`_UNIVERSITY_PLAYLISTS.md`](_UNIVERSITY_PLAYLISTS.md) §session-6.

**Session-6 GRAND TOTAL: 29 new courses · 998 lecture transcripts.** `corpus/courses/` now holds **91 full lecture courses, 2,615 transcripts, 96 MB**. All legally-free (course-public YouTube auto-subs · MIT OCW CC BY-NC-SA · expert-channel public captions). All text git-ignored.
...
**Totals (after gathering session 6, 2026-06-29):** **~1,672 sources** (1,643 + 29 new university/expert-channel courses [998 lecture transcripts]) · `corpus/courses/` now holds **91 full lecture courses, 2,615 transcripts, 96 MB** — widened past the AI/ML spine into the **under-served domains** (probability, quantum, physics, economics, biology, robotics/control, neuroscience, cognitive-science, applied-math/calculus, PGM) across **4 newly-represented universities** (Harvard STAT110, UMich EECS498, Cornell CS4780, Tübingen DL) + new Berkeley/CMU/MIT courses + 2 expert channels (Steve Brunton control/dynamics, Artem Kirsanov neuro). Every playlist ID validated live before queueing; no 429 storms; benign empties documented. All text git-ignored. **Next:** further widening if wanted (NPTEL/IIT math-physics-EE, Princeton/UW/Oxford/EPFL/Toronto if clean lecture playlists surface, StatQuest stats, MIT TinyML 6.5940 / scientific-ML 18.337 on lab channels) — **or**, once the learner calls the corpus rich enough, the first **grounded module rewrite** (1300 LLMs). Method ▶ [`_CORPUS_BUILD.md`](_CORPUS_BUILD.md).
