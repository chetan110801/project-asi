...
- **Video/YouTube + podcasts (transcripts, v2.4):** fetch the **transcript/captions** → `corpus/transcripts/<source>/`. Store text only; note speaker + date. Use these for intuition/the "why" and **ground factual claims to a primary source** (HARD_RULES §1.3, §2). Transcripts are for local note-writing only (stay git-ignored).
  - **HTML transcript pages** (Lex Fridman `lexfridman.com/<guest>-transcript/`, Dwarkesh Substack `dwarkesh.com/p/<slug>` — full transcript is in the page HTML; enumerate all posts via `dwarkesh.com/sitemap/<year>.xml`): `curl | html2txt.pl` then `chunk.sh` (helper `getts.sh`).
  - **YouTube auto-subs** (built session 3): `yt-dlp` installed in `…/Programs/Python/Python314` — in Git Bash call via the **full python path** (`python` alone = Win-Store stub). List a channel/playlist: `python -m yt_dlp --flat-playlist --print "%(id)s|%(title)s" <url>`. Per video: `python -m yt_dlp --skip-download --write-auto-subs --sub-langs en --sub-format vtt -o "<id>.%(ext)s" <watch_url>` → clean the VTT with **`vtt2txt.pl`** (drops `WEBVTT`/cue-numbers/`-->` timestamp lines + inline `<…>` tags, de-dupes the rolling-caption repeats). **Be polite:** ONE sub-lang per video (asking many variants → HTTP 429), `--sleep-requests`, base sleep ≥2.5 s, back off 45–60 s on 429. The driver **`ytchannel.sh <videolist> <outdir> <label>`** wraps this and is **resumable** (`.done_<id>` markers + skip existing) — safe to re-run to finish a partial channel. University course catalog: [`_UNIVERSITY_PLAYLISTS.md`](_UNIVERSITY_PLAYLISTS.md).
