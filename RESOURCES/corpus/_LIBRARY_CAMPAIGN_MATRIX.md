...
- ✅ **+ Session 14:** **OpenStax College Physics 2e** (algebra-based L1, `354`) — fills the algebra-based intro tier under the calculus-based University Physics 1–3. ⬜ L4: arXiv hep/cond-mat/astro-ph reviews.
...
- ✅ **+ Session 13:** **Kellis (ed.) — Computational Biology: Genomes, Networks, Evolution** (MIT 6.047/6.878 open textbook, compiled PDF, `101` chunks → `biology/kellis-computational-biology/`) — fills **systems/computational biology** · **Boundless Microbiology Ch.11 Immunology** (LibreTexts, `5` chunks → `biology/libretexts-immunology/`) — dedicated **immunology** top-up on the held OpenStax Microbiology immunity chapters.
- ✅ **+ Session 14:** **Evolutionary Developmental Biology** (Rivera, LibreTexts, CC BY-NC-SA, `21`) — body-plan evolution, Hox/regulatory genes, fins-to-limbs, eye evolution, evo-devo of novelty — fills **developmental biology / evo-devo**. **[Session-12: genetics. Session-13: computational-biology + immunology. Session-14: developmental biology filled — biology sub-domains now end-to-end.]**
...
- ✅ **+ Session 14:** **Dive Into Systems** (Matthews/Newhall/Marlow, free CC, `41`) — fills **computer architecture / OS / memory hierarchy / concurrency / parallel-&-GPU** (concept chapters only; C-programming + assembly-language + command-line tutorials skipped per the durability filter).
- ⬜ Remaining: Security ("Computer Security" open), Software-engineering open texts. **[Session-10: algorithms, cryptography, compilers/PL, formal verification, graphics. Session-14: computer architecture & systems.]**
...
- ✅ **+ Session 13:** **Kirtley — Introduction to Electric Power Systems** (MIT 6.061, Engineering LibreTexts, `27` chunks → `electrical-engineering/kirtley-electric-power-systems/`) — fills **power systems** · **Lee & Seshia — Introduction to Embedded Systems: A Cyber-Physical Systems Approach 2e** (free, LeeSeshia.org, `79` chunks → `electrical-engineering/lee-seshia-embedded-systems/`) — fills **embedded systems** (models of computation, state machines, concurrency, real-time, sensors/actuators). + **MIT 6.013 Electromagnetics** (course).
- ⬜ Remaining: **Communications** (Proakis/Haykin → REQUESTS; partial coverage held = MacKay info-theory + Baraniuk signals) · **VLSI** (Weste&Harris → REQUESTS). **[Session-10: electromagnetics. Session-12: signals-&-systems + circuits. Session-13: power-systems + embedded filled; comms + VLSI → REQUESTS.]**
...
- ✅ **+ Session 13:** **USGS Open-File Report 88-707 — Basic Hydraulic Principles of Open-Channel Flow** (US Geological Survey, public domain, `19` chunks → `civil-engineering/usgs-open-channel-flow/`) — energy/momentum equations, uniform & gradually-varied flow, velocity profiles, flow resistance — fills **hydraulics / open-channel flow** (also supported by Bar-Meir fluid mechanics).
- ✅ **+ Session 14:** **Fundamentals of Transportation** (LibreTexts, CC BY-SA, `38`) — planning models, trip generation/distribution, mode/route choice, transit, traffic flow & control, geometric design — fills **transportation engineering**.
- ⬜ Remaining: deeper water-resources (lower priority). **[Session-11: structural analysis. Session-12: geotechnical/foundation. Session-13: hydraulics/open-channel-flow. Session-14: transportation filled.]**
...
- ✅ **+ Session 14:** **Chemical Engineering Separations: A Handbook for Students** (Lamm & Jarboe, LibreTexts, CC BY, `8`) — distillation, absorption/stripping, extraction, McCabe-Thiele — fills **separations/distillation**.
- ⬜ Remaining: transport phenomena, reaction engineering, process design. **[Session-11: ChemE intro + process control. Session-14: separations/distillation filled.]**
...
- ✅ **+ Session 13:** **George (ed.) — Introduction to Orbital Mechanics** (CU Boulder, Pressbooks OER, `25` chunks → `aerospace-engineering/intro-orbital-mechanics/`) — Kepler's laws, two-body problem, orbital elements, launch windows, maneuvers, ground tracks/rendezvous, perturbations, interplanetary transfer — fills **orbital mechanics/astrodynamics**. + **MIT 16.885J Aircraft Systems Engineering** (course).
- ✅ **+ Session 14:** **Fundamentals of Aerospace Engineering** (Arnedo, LibreTexts, `27`) — a compact end-to-end text spanning aerodynamics, **propulsion** (jet/rocket cycles), flight mechanics, structures & orbital mechanics — adds the **propulsion** coverage on top of Marchman/Leishman/George.
- ⬜ Remaining: a dedicated standalone propulsion monograph (low priority — Arnedo + Leishman now cover it). **[Session-11: aerodynamics + aircraft performance + flight-vehicles. Session-13: orbital mechanics. Session-14: propulsion via Arnedo.]**
...
### Biomedical / Bioengineering — `biomedical-engineering/` (NEW — folder seeded Session 14)
...
- ✅ Held: Yale Frontiers of Biomedical Engineering 🔄; MIT 6.S897 ML-for-Healthcare (course); owned AI-healthcare volumes. **+ Session 14:** **Introduction to Biosystems Engineering** (Holden et al., LibreTexts, CC BY, `91`) — sensors & mechatronics, bioprocess engineering, instrumentation/controls, machinery automation, agricultural/biological systems — seeds the `biomedical-engineering/` book folder.
- ⬜ Remaining: dedicated biomechanics, medical-imaging, biosignals texts (lower priority). **[Session-14: biosystems/bioprocess engineering — folder seeded.]**
...
- **Session 13 ✅ DONE (2026-07-01) — 2nd L4 arXiv batch + residual books + MIT-OCW gap courses.** **(A)** 16 arXiv surveys / 249 chunks → `papers/D13/` (NLP/speech/object-detection/image-segmentation/meta-learning/transfer/uncertainty/OOD/neuro-symbolic/NAS/PEFT/prompting/hallucination×2/offline-RL/multiagent-RL) — every ID API-searched then re-verified (0 mismatches; running total ≈196). **(B)** 6 free books / 256 chunks: Kellis Computational-Biology (MIT 6.047) + Boundless Immunology (`biology/`); Kirtley Electric-Power-Systems (MIT 6.061) + Lee&Seshia Embedded-Systems (`electrical-engineering/`); George Intro-Orbital-Mechanics (`aerospace-engineering/`); USGS Open-Channel-Flow hydraulics (`civil-engineering/`). Comms (Proakis/Haykin) + VLSI (Weste&Harris) → REQUESTS (free partial = MacKay + Baraniuk). **(C)** 4 MIT-OCW courses / 137 transcripts: 6.013 Electromagnetics `24`, 16.885J Aircraft-Systems `22`, 8.701 Nuclear-&-Particle-Physics `62` (4 transient-429 left for re-run), 18.086 Math-Methods-II `29` → `courses/` now **134 folders**. All 0 FFFD; all text git-ignored.
- **Session 14 ✅ DONE (2026-07-01) — residual-book wave + 8.701 recovery + NEW Stream 3 (Frontier R&D).** **(A)** Recovered all **4 transient-429 MIT 8.701 clips** (course now 66 transcripts). **(B) 7 free books / 580 chunks** (all 0 FFFD) filling the last thin sub-domains: OpenStax **College Physics 2e** `354` (physics L1, algebra-based) · **Fundamentals of Transportation** (LibreTexts, `38` → civil **transportation**) · **ChemE Separations: Handbook for Students** (Lamm & Jarboe, LibreTexts, `8` → ChemE **separations/distillation**) · **Fundamentals of Aerospace Engineering** (Arnedo, LibreTexts, `27` → aerospace incl. **propulsion**) · **Evolutionary Developmental Biology** (Rivera, LibreTexts, `21` → **developmental biology**) · **Introduction to Biosystems Engineering** (Holden et al., LibreTexts, `91` → NEW `biomedical-engineering/` folder) · **Dive Into Systems** (diveintosystems.org, `41` → CS **computer architecture**, concept chapters only — coding/assembly tutorials skipped). **(C) 🆕 NEW LEARNER DIRECTIVE → Stream 3 = Frontier R&D corpus** (companies + universities + organizations × every field, Neuralink included) → see ⭐ [`_FRONTIER_RND_CORPUS.md`](_FRONTIER_RND_CORPUS.md); first wave = **27 sources / 342 chunks → NEW `frontier-rnd/`** (Neuralink whitepaper+brochure, 12 full-text MIT-News field feeds, BAIR/Google/Amazon/NVIDIA/Apple/HF/Meta/MIT-CSAIL, IEEE Spectrum, robotics/quantum snapshots). Built `rss2txt.pl`+`rndfeed.sh`; hardened `chunk.sh` (3× hard-cap).
- **Session 15+ — continue both streams.** Stream 2: residual ⬜ (machine design/manufacturing/CFD, transport phenomena/reaction-eng, nanomaterials, astrophysics notes, hydrology, linguistics; OCR the 3 scans if a stack is added) — **low priority, corpus is broad.** Stream 3: work [`_FRONTIER_RND_CORPUS.md`](_FRONTIER_RND_CORPUS.md) §2 (YouTube talk channels one-at-a-time · MSR/IBM/AI2/Anthropic/OpenAI/Stanford-HAI/CMU/national-labs/Allen-Institute/CERN feeds · NASA NTRS report PDFs · quantum/semiconductor/fusion/biotech/space sectors). **Then pause for the learner's "rich enough" call → first grounded module rewrite (1300 LLMs).**
