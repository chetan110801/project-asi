...
>
> **This page builds on rungs you have already climbed**, each with a short plain reminder where it is used, so you will not get lost: [scaling laws & the data wall](../10-how-ai-works-today/02_scaling-laws-and-emergence.md) — the steady line "make it bigger" rides, and the fact that good text is finite; and [AP1 · the "make it bigger" bet](../20-the-approaches/01_ap1-scale-and-foundation-models.md) — the mainstream bet these walls press on. **This page is the deep home for the three bounds: the data stock, the compute trend and its cost, and the energy wall.** The map's closing page, [The verdict](../40-the-verdict/01_which-bets-get-to-agi.md), is read *after* this one and leans on these three walls — so read them well here; they become one of its gauges.
>
...
Three quick reminders from pages you have already read, so this page stands on its own.
...
- From [AP1](../20-the-approaches/01_ap1-scale-and-foundation-models.md): the **"make it bigger" bet** stands on that steady line, backed by the **Bitter Lesson** *(Sutton's finding that, across AI's whole history, methods that just use more computer power keep beating methods that build in clever human knowledge by hand)*. AP1 also set the map's central question — **is scale the *source* of intelligence, or only the *fuel*?** (the *scaling-suffices debate* — whether making models bigger is by itself enough to reach AGI). These walls are the physical price of that bet, and the arithmetic side of that question.
...
- **They feed the [verdict's](../40-the-verdict/01_which-bets-get-to-agi.md) judgement, which you read after this page.** The verdict watches two "gauges" — the **novelty gap** and the **data wall** — to decide whether scale keeps paying off. This page is the deep version of the data-wall gauge, plus two more dials beside it (compute cost, energy). If these three keep receding, the mainstream stack keeps improving; if they bind, it stalls. So the bounds are *how you will actually read* the verdict's central "does scale keep buying gains?" question when you get there.
- **They explain why the data-wall answers (AP4 / AP9 / AP10) are rising.** Those three bets look like rivals to scale, but Wall 1 shows they are really where scale's *next* training experience comes from once human text is spent. The energy wall raises them further: experience and self-play can be far cheaper per unit of learning than reading the whole internet again.
...
- **Down the ladder (already read):** [scaling laws & the data wall](../10-how-ai-works-today/02_scaling-laws-and-emergence.md) — owns the *idea* of the data wall (this page adds the *quantities*); [AP1 · scale](../20-the-approaches/01_ap1-scale-and-foundation-models.md) — the bet the walls press on.
- **Up the ladder (read after this):** [The verdict](../40-the-verdict/01_which-bets-get-to-agi.md) — reads last and leans on these three walls; it watches the data wall as one of its two gauges, and this page gives that gauge (and the two beside it) their numbers.
...
6. [AP1](../20-the-approaches/01_ap1-scale-and-foundation-models.md) asked "is scale the *source* or the *fuel*?" What second, colder question do the bounds add underneath it? (And which page, read after this one, folds that question into its final judgement?)

## Revision notes

*Newest first.*
- `rev 2 (2026-07-16)` — **reconciled the reading order: this page reads *before* the verdict, not after.** Rev 1 was written treating [The verdict](../40-the-verdict/01_which-bets-get-to-agi.md) as already-read (it was in the prereqs, the "you are here" builds-on list, the base-reminder, and several "the verdict's two gauges / the verdict called them the data-wall answers" back-references). But the map's navigation ([spine](../APPROACHES_TO_AGI.md) "New here" box + `build_site.py` GROUP_ORDER) places the verdict **last** — after this cross-cutting page — and the verdict's own rev 2 now *leans on* this page. To remove the circular order, this rev: **dropped `c-verdict` from the prereqs**; changed "you have read the eleven bets **and the verdict**" → "you have read the eleven bets"; **re-sourced** the two verdict-originated ideas to their true homes — the *"is scale the source or the fuel?"* question now credits [AP1](../20-the-approaches/01_ap1-scale-and-foundation-models.md)'s scaling-suffices debate, and the **data-wall answers** grouping (AP4/AP9/AP10) is now introduced here on its own (the data page owns it), with a note that the verdict re-uses the name; and turned every remaining verdict mention into a **forward**-reference ("read after this page," "up the ladder," "feeds the verdict's judgement"). No factual content changed — only the direction of the cross-links, so the ladder reads cleanly bottom-to-top. **§7.0 recheck done:** re-read whole file; confirmed no line still assumes the verdict was already read; all internal links still resolve; the four verbatim quotes and every dated number are untouched.
