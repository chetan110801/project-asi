...
level: core
prereqs: [c-ap4-rl, c-ap9-open-endedness, c-ap11-whole-brain-emulation, c-bounds]
...
- From the **eleven approach cards**: every one asked the same kind of question — *can this bet build a general mind?* (*does it work?*). This page adds the question **none** of them asked: *and can we steer whatever gets built?* *(The [verdict](../40-the-verdict/01_which-bets-get-to-agi.md), read after this page, ranks the bets on that first question, then leans on this page for the second ruler.)*
...
- **It gives the verdict its second ruler.** [The verdict](../40-the-verdict/01_which-bets-get-to-agi.md), which you read *after* this page, ranks the bets by *how likely each is to be part of the path to AGI* — the *does it work?* axis. This page is the source of the second ruler it leans on: *and can we steer whatever gets there — and survive its self-improvement?* A full view of the map needs both rulers: **does it work**, and **does it go well.** (The verdict's own closing turns this into a three-question read: does it work, can we afford it, can we control it.)
...
- **Its links across the map:** [The verdict](../40-the-verdict/01_which-bets-get-to-agi.md) — read *after* this page; this page is the source of the second ruler (can we steer it?) that the verdict lays beside its ranking (does it work?); the **mainstream stack** (AP1+AP2+AP3+AP4) — where alignment presses hardest because it is most capable; [the scaling-suffices debate](../20-the-approaches/01_ap1-scale-and-foundation-models.md) — a fast RSI take-off would be scale answering "enough?" by out-building us.
...
*Newest first.*
- `rev 2 (2026-07-16)` — **reconciled the reading order: this page reads *before* the verdict, not after.** Rev 1 was written treating [The verdict](../40-the-verdict/01_which-bets-get-to-agi.md) as already-read (it was in the prereqs, the "you are here" builds-on list, a base-reminder bullet, and the "it completes the verdict" bullet). But the map's navigation ([spine](../APPROACHES_TO_AGI.md) "New here" box + `build_site.py` GROUP_ORDER) places the verdict **last** — after this page — and the verdict's own rev 2 now *leans on this page* for its control axis. To remove the circular order, this rev: **dropped `c-verdict` from the prereqs** (kept `c-bounds`, which does read before this page); changed "you have read the eleven bets, **the verdict**, and the bounds" → "you have read the eleven bets and the bounds"; **re-sourced** the base-reminder's *does-it-work* framing to the **eleven approach cards** themselves (every card asked it) instead of the verdict's ranking; and turned every verdict mention into a **forward**-reference — "read after this page," and *"this page is the source of the second ruler the verdict leans on"* (which now matches the verdict's new three-axis closing: does it work · can we afford it · can we control it). Also dropped a forward-link on "mainstream stack" (kept the plain AP1+AP2+AP3+AP4 inline). No factual content, quote, or dated number changed — only the direction of the cross-links. **§7.0 recheck done:** re-read whole file; confirmed no line still assumes the verdict was already read; all eight verbatim quotes and every dated finding untouched; all internal links resolve.
