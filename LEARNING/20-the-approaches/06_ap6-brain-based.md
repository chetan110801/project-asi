...
> **▶ Go deeper — one deep dive past this card.** This card named "the predictive brain" and "brain-style chips" as slogans, and admitted (Stuck #1) that nobody has the full cortical algorithm. **[AP6 · Deep dive #1: predictive coding & the learning rule](../50-deep-dives/10_ap6-deep-dive-predictive-coding-and-the-learning-rule.md)** opens the concrete mechanisms the card left as slogans — and names the one gap it skipped entirely. **Predictive coding** is the real theory under "a column predicts its next input": the cortex sends **predictions down** and passes **only errors up**, so perception is a guess corrected by surprise. The **free-energy principle** is its grand form — the brain minimises one quantity, *surprise*, by perceiving or acting. And the gap the card never mentions is **the learning rule**: every capable AI learns by **backpropagation**, which the brain almost certainly **can't run** (synapses are one-way; biology learns *locally*) — yet **predictive coding can approximate backprop using only local updates**, so the brain can *match*, not beat, the algorithm AI already uses. The same local, spiking style in silicon (**neuromorphic** chips) is 25–100× more efficient but **stranded** by the training gap. Its verdict flips this card's Stuck #1: the concrete question is no longer "what is the cortical algorithm?" but **"is the brain's difference a new capability, or just a better implementation?"** — and 2026's evidence points at implementation. Optional — read it for the machinery, and the one clean brain-vs-AI difference.
...
- **Go deeper (⑤, optional):** [AP6 · Deep dive #1: predictive coding & the learning rule](../50-deep-dives/10_ap6-deep-dive-predictive-coding-and-the-learning-rule.md) — the *mechanisms* under the card's slogans: **predictive coding** (predictions down, errors up — perception as a corrected guess), the **free-energy principle** (minimise *surprise* by perceiving or acting), and the gap this card skips — **the learning rule** (the cortex can't run **backpropagation**; it learns *locally*, and predictive coding can *approximate* backprop with only local updates → the brain *matches*, not beats, AI's algorithm); plus **spiking / neuromorphic** hardware (efficient but stranded). Its big question sharpens Stuck #1: is the brain's edge a new **capability** or just a better **implementation**? (2026 evidence leans implementation.)
