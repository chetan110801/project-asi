...
- **The ideas it leads to** (still to be written): AP4 (RL from interaction) gives the *reward-driven* version of the agent — an agent that learns its loop from reward in a world, rather than being scripted; AP7 (neurosymbolic) and AP9 (open-endedness) bear on how an agent could *plan* and *keep learning* reliably. See the [map](../APPROACHES_TO_AGI.md).
