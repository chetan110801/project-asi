...
> **▶ Go deeper — one deep dive past this card.** This card gave the four pieces (act-loop/tools, memory/reflection, planning, skill-library) as **ideas** — a diagram of parts. **[AP3 · Deep dive #1: inside the agent loop](../50-deep-dives/08_ap3-deep-dive-inside-the-agent-loop.md)** opens the **machine** under them. It shows that the model does **not** run the loop (a plain program, the **harness**, does — the model is *stateless*, called fresh each turn and forgetting in between); that a "tool" is really a **wish written in structured text** that separate code executes, not the model reaching out (with **MCP** the 2026 standard plug); and that the agent's entire world each turn is **one finite page — the context window** — which *degrades as it fills*, so all four "pieces" turn out to be operations fighting over that one scarce page (the craft has a name: **context engineering**). It ends on the 2026 verdict that the long-task wall is a **context-handling gap, not a reasoning gap**, and judges the loop's *own* four cracks — the finite window everything competes for · a harness that can never be absorbed into a stateless call · tool-calling's own march of nines · memory that is lossy retrieval, not recall. Optional — read it when you want the assembly line under the architecture.
...
- **Go deeper (⑤, optional):** [AP3 · Deep dive #1: inside the agent loop](../50-deep-dives/08_ap3-deep-dive-inside-the-agent-loop.md) — the *machine* under the four pieces: the model is **stateless** and a plain program, the **harness**, runs the loop; a "tool" is a **structured-text wish** other code executes (MCP is the standard plug); and the agent's whole world each turn is **one finite context window** that degrades as it fills, so memory/planning/tools/skills are all really **context management** (the craft = context engineering). The 2026 long-task wall is a **context-handling gap, not a reasoning gap** — and the loop's own four cracks are the finite window everything competes for · an un-absorbable harness · tool-calling's own march of nines · lossy memory.
