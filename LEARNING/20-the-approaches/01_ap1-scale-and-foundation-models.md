...
> **▶ Go deeper — one deep dive past this card.** This card drew the "make it bigger" bet as a **straight, predictable line** (and the [scaling-laws rung](../10-how-ai-works-today/02_scaling-laws-and-emergence.md) gave the picture). **[AP1 · Deep dive #1: the anatomy of a scaling law](../50-deep-dives/07_ap1-deep-dive-anatomy-of-a-scaling-law.md)** opens the **equation** under that line: the three-piece loss formula (`loss = an unbeatable floor + a size-penalty you pay down with parameters + a data-penalty you pay down with tokens`) and the compute identity (`compute ≈ 6 × parameters × tokens`) that turns a budget into a machine shape — so "how big, trained on how much?" becomes a solved math problem. It shows how labs **read the future off the line** (GPT-4's score was predicted from models using **1/1,000th** the compute), tells the true story of the **two years the equation was measured wrong** and misdirected the whole field, and reaches the deepest fact — we use the law with great precision and **cannot explain why it holds**. Then it judges the *instrument's own* four cracks — it is **measured, not derived** (no theory); it predicts **loss, not capability**; its exponents make progress **exponentially costlier**; and it was **mis-measured once**, for years — so stretching the line out to AGI is a leap of faith, not a calculation. Optional — read it when you want the machine beneath the line.
...
- **Go deeper (⑤, optional):** [AP1 · Deep dive #1: the anatomy of a scaling law](../50-deep-dives/07_ap1-deep-dive-anatomy-of-a-scaling-law.md) — the *equation* under the "make it bigger" line: the three-piece loss formula + the `6ND` compute identity (a budget → a solved shape), how labs fit-and-extrapolate it to predict a giant model from 1/1,000th the compute, the two-year measurement bug, and the instrument's own four cracks (measured-not-derived · loss≠capability · cruel exponents · mis-measured once) — so extrapolating it to AGI is a leap of faith.
