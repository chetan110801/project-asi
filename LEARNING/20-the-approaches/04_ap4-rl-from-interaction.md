...
> **▶ Go deeper — a deep dive past this card.** This frame *names* the policy and the value but never shows **how they actually improve**, step by step. That machine — the **engine** of reinforcement learning — is opened in **[AP4 · Deep dive #1: the engine](../50-deep-dives/04_ap4-deep-dive-the-engine.md):** how a machine learns value by **"a guess from a guess"** (**temporal-difference learning**, and the **TD error** that quietly solves credit assignment) — the very signal real **dopamine** neurons turn out to fire; the **two families** of turning learning into action (**value-based** Q-learning/DQN vs **policy-based** methods) and the **actor-critic** marriage that wins (**PPO**); the **model-free vs model-based** fork that explains *why* RL is so hungry for experience (and hands the model-based branch to [AP5](05_ap5-world-models-jepa.md)); and the punchline — this exact engine is what trains 2026's chatbots (**RLHF**) and reasoning models (**GRPO**). Optional — read it when you want the machinery beneath "trial and error."
>
> **▶ And a second deep dive #2 — the reward, not the engine.** Deep dive #1 settles the *engine* and then sketches how it trains today's models; **[AP4 · Deep dive #2: the reward is the whole game](../50-deep-dives/16_ap4-deep-dive-the-reward-is-the-whole-game.md)** opens that sketch into the whole frontier — the actual **post-training pipeline** (SFT → learned **reward model** → optimize) that turns a raw predictor into a helpful assistant; why the reward is a **proxy** you must leash (over-optimization → **sycophancy**, which shipped to millions in 2025); and the frontier's **march on the reward** (human preference → an AI's preference / **Constitutional AI** → **DPO**, no reward model → **verifiable rewards** that grew DeepSeek-R1's reasoning) where every step just **relocates** the cheat. It ends by making this card's big question — *is reward the **source** of intelligence or a **tool**?* — measurable: does RL **teach** a mind or only **reveal** the one pre-training built? *(As of 2026: mostly reveal.)*
...
- **▶ Go deeper (deep dives):** [#1 · the engine](../50-deep-dives/04_ap4-deep-dive-the-engine.md) — the machine beneath this card's "trial and error": **temporal-difference learning** (learn "a guess from a guess"; the **TD error = surprise** that solves credit assignment, and the **dopamine** signal that matches it), **value-based vs policy-based** methods and the **actor-critic** workhorse (**PPO**), **model-free vs model-based** RL (**MuZero**; *why* RL is sample-hungry, handed on to [AP5](05_ap5-world-models-jepa.md)), and how this engine trains 2026's models (**RLHF/PPO → verifiable-reward/GRPO**). And [#2 · the reward is the whole game](../50-deep-dives/16_ap4-deep-dive-the-reward-is-the-whole-game.md) — the **reward layer** above the engine: the real **post-training pipeline** (SFT → **Bradley-Terry** reward model → optimize, on a **KL leash**); the reward as a gameable **proxy** (over-optimization → **sycophancy**); the **march on the reward** (RLHF → **RLAIF/Constitutional AI** → **DPO** → **verifiable rewards / DeepSeek-R1**) where every escape relocates the cheat; and the 2026 verdict via **pass@k** that RL mostly **reveals** rather than **teaches** — the card's "source or tool?" answered toward *tool*. Two optional layers past this card.
- **The ideas it leads to:** now written — [AP5 · world models](05_ap5-world-models-jepa.md) (learn a model of the world to *plan* inside, easing AP4's hunger for real tries), [AP3 · agents](03_ap3-agents-and-cognitive-architectures.md) (wrapping a model in a goal-seeking loop), and the [alignment thread](../30-across-the-approaches/02_alignment-control-and-self-improvement.md) (a reward-maximizer you cannot fully control is the core safety worry — AP4's reward hacking is its first layer). See the [map](../APPROACHES_TO_AGI.md).
