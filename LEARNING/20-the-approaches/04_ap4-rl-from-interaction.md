...
- **The ideas it leads to** (still to be written): AP5 (world models — learn a model of the world to *plan* inside, easing AP4's hunger for real tries), AP3 (agents — wrapping a model in a goal-seeking loop), and the alignment thread (a reward-maximizer you cannot fully control is the core safety worry). See the [map](../APPROACHES_TO_AGI.md).
