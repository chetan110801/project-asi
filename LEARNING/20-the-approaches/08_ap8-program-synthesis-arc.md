...
> **▶ Go deeper — three deep dives past this card.** **[#1 · the formal theory of intelligence](../50-deep-dives/01_ap8-deep-dive-the-formal-theory-of-intelligence.md):** the one-line definition above (*skill-acquisition efficiency*) is the tip of an actual **formula** — why you can only ever measure *skill* (and how it is *bought*), the **conversion-rate fraction** behind "efficiency," the length-of-the-shortest-program ruler, the **ladder of generalization**, the **four innate core-knowledge systems**, and why Chollet says there is *no universal intelligence*. **[#2 · how ARC is actually solved](../50-deep-dives/02_ap8-deep-dive-solver-mechanisms.md):** the real machinery behind Part 3's "guided program search" — **DreamCoder** (a system that grows its own toolkit and learns to search it), **induction vs transduction** (write-a-program vs paint-the-grid — they solve *different* puzzles), the **test-time-training** recipe that won 2024 (augment → fine-tune per puzzle → vote), and the 2025 **refinement loop**. **[#3 · the research frontier](../50-deep-dives/03_ap8-deep-dive-the-research-frontier.md):** the open problems those tricks left behind, and where the road *merges* — growing the priors in an **open world**, nets that **propose whole programs** (latent-space search, and the **compositionality** wall), **efficiency as the problem statement** (the cost-capped competition), the **interactive** turn (**ARC-AGI-3**, where you must *explore* to find the puzzle), and program search that **discovers new mathematics** (**FunSearch**) — the bridge into [AP9 · open-endedness](09_ap9-open-endedness.md). All optional — read them when you want the depth beneath the slogans.
...
- **▶ Go deeper (deep dives):** [#1 · the formal theory of intelligence](../50-deep-dives/01_ap8-deep-dive-the-formal-theory-of-intelligence.md) — the *Measure of Intelligence* formula beneath this card's slogan (the conversion-rate fraction, the program-length ruler, the generalization spectrum, the four core-knowledge systems, the no-universal-intelligence fight); [#2 · how ARC is actually solved](../50-deep-dives/02_ap8-deep-dive-solver-mechanisms.md) — the solver machinery beneath Part 3 (DreamCoder's wake–sleep library learning, induction vs transduction, test-time training with an augmentation vote, the refinement loop); and [#3 · the research frontier](../50-deep-dives/03_ap8-deep-dive-the-research-frontier.md) — the open problems left behind and where the road merges (growing priors in an open world, latent-space program proposal + the compositionality wall, efficiency as the problem statement, the interactive **ARC-AGI-3** turn, and **FunSearch** discovering new mathematics — the bridge to [AP9](09_ap9-open-endedness.md)). The optional layers past this card.
...
- **Its wider family & where it points:** AP8 is the **ARC-focused special case** of [AP7 · neurosymbolic / hybrid AI](07_ap7-neurosymbolic-and-hybrid-ai.md) — the general "join a neural learner to a symbolic reasoner / search" bet you read just before this (AP8's own "deep-learning-guided program search" *is* a neurosymbolic architecture); and it connects to [AP6 · brain-based](06_ap6-brain-based.md) (how the one working mind we have gets its priors — Stuck #1). And [AP9 · open-endedness](09_ap9-open-endedness.md) (now written) takes up exactly that: where do broad priors *come from*? maybe an endless generating process. See the [map](../APPROACHES_TO_AGI.md).
