...
> **▶ Go deeper — one deep dive past this card.** This card gave "think longer" as a single dial and one rising curve. **[AP2 · Deep dive #1: how a machine actually thinks longer](../50-deep-dives/09_ap2-deep-dive-how-a-machine-thinks-longer.md)** opens the machine under it. "Spend compute at answer-time" turns out to be a whole **family** of methods: think **longer** (one long chain), think **wider** (many chains, then vote or pick — *self-consistency*), or **search** a tree of half-thoughts (*Tree of Thoughts*). And it reveals the split the single curve hides: **generating** a right answer is nearly solved (sample a hard problem enough times and a correct answer almost always appears — "coverage" scales as a power law), but **finding** it is the ceiling — without an automatic checker, majority-voting and reward models **plateau**. So the whole bet rests on a power the card never named — **verification** (checking is easier than generating; grading each *step*, a process reward model, beats grading the end). It closes on the compute-optimal science (a small model that thinks well can beat a **14×** larger one — but past a knee, more thinking *overthinks* and hurts) and judges the machine's own four cracks: **the bottleneck is verification, not generation** · the verifier is itself a fallible, gameable model · both curves bend · the AlphaZero template transfers but its perfect checker does not. Optional — read it to see why the frontier of "think longer" is really a **verification** problem.
...
- **Go deeper (⑤, optional):** [AP2 · Deep dive #1: how a machine actually thinks longer](../50-deep-dives/09_ap2-deep-dive-how-a-machine-thinks-longer.md) — the *machine* under "think longer": **sequential** (one long chain) vs **parallel** (many chains + a pick) vs **search** (Tree of Thoughts); the **coverage-vs-selection** split (generating a right answer scales as a power law, but *finding* it without a checker **plateaus**); the **verifier** as the hidden engine (checking is easier than generating; **process** rewards beat **outcome** rewards); and the **compute-optimal / overthinking** curve (a small model that thinks well can beat a 14× larger one, yet more thinking eventually *hurts*). Its four cracks: verification-is-the-bottleneck · the verifier is a fallible model · the curves bend · AlphaZero's perfect checker doesn't transfer.
...
- **The ideas it leads to** (now written — read them): [AP4 · RL from interaction](04_ap4-rl-from-interaction.md) (the deep version of Leg 3's reward-learning), [AP8 · program synthesis / ARC](08_ap8-program-synthesis-arc.md) (Chollet's own idea — handling truly new problems, the charge behind Stuck #4). See the [map](../APPROACHES_TO_AGI.md).
