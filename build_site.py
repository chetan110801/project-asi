...
GROUP_ORDER = [
    ...
    ("60-entry-ramps", "⑥ Entry ramps — how to learn each approach"),
]
...
/* The controls are deliberately INVISIBLE (learner's request): fully transparent
   at rest, on hover, and on touch — but still laid out, still hit-testable, and
   still the same 40/44px targets, so tapping or clicking the top-left corner
   works exactly as before on every device. `opacity:0` (not `visibility` or
   `display`) is what keeps them clickable. The only thing that ever reveals
   them is keyboard focus, which no mouse or touch reader can trigger — without
   it a keyboard user would be tabbing onto controls they cannot see. */
.navbar button{-webkit-appearance:none;appearance:none;border:0;background:transparent;
  color:var(--muted);font-size:1.05rem;line-height:1;cursor:pointer;opacity:0;
  display:flex;align-items:center;justify-content:center;gap:.3rem;
  min-width:40px;height:40px;border-radius:9px;transition:opacity .18s,background .18s,color .18s;
  -webkit-tap-highlight-color:transparent;touch-action:manipulation}
.navbar button:hover{opacity:0}
.navbar button:focus-visible{opacity:1;color:var(--text);
...
@media (hover:none){.navbar button{opacity:0}}
...
(function(){
  ...
  // (The old 2.6s "hint" flash that briefly lit the nav cluster on load is gone —
  //  the controls are invisible by design now, so flashing them contradicted it.)
